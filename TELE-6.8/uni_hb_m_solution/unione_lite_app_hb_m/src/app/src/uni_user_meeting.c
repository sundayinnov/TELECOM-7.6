...
int uni_user_meeting_send_event(USER_EVENT_TYPE event,
  ...
  switch (event) {
    ...
    case USER_GOTO_AWAKENED:
      ...
      break;
    // 新增学习事件处理（目前无动态内存，如需将来扩展可添加 CHECK_FREE）
    case USER_STUDY_START_EVENT:
      // 如果未来 study_start.cmd 为动态分配，需释放
      // CHECK_FREE(context->study_start.cmd);
      break;
    case USER_STUDY_SUCCESS_EVENT:
      // 无动态内存
      break;
    case USER_STUDY_FAIL_EVENT:
      // 如果未来 study_end.reason 为动态分配，需释放
      // CHECK_FREE(context->study_end.reason);
      break;
