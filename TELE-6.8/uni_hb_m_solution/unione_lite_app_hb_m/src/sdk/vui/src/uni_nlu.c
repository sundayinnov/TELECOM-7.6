...
cJSON* NluParseLasr(const char *key_word) {
  cJSON *result = NULL;
  uni_u32 hashCode = uni_hash(key_word);
  LOGT(TAG, "%s %u", key_word, hashCode);
  ...
  uni_u8 study_nlu_index;
  if (StudySessionGetNluIndex(hashCode, &study_nlu_index)) {
    LOGT("NLU", "found study result %s", g_nlu_content_str[study_nlu_index]);
    result = cJSON_Parse(g_nlu_content_str[study_nlu_index]);
    if (result == NULL) {
      LOGE("NLU", "json parse failed, memory may not enough, reboot now");
      WatchDogReboot();
      return NULL;
    }
    _replace_asr_recongize(result, key_word);
