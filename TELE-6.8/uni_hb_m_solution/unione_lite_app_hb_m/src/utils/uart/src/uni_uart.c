...
void _recv_uart_data_handler(UART_PORT_T port) {
  char c = 0;
  DataBufHandle databuf;
...
// 新增：导出 HAL 接收回调函数指针（供外部硬件重配使用）
void* uni_uart_get_recv_cb(void) {
    return (void*)_recv_uart_data_handler;  // _recv_uart_data_handler 原为 static，需去掉 static
}
