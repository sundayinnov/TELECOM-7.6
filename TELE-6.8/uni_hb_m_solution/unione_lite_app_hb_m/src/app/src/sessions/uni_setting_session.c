...
#define range  MEDIA_VOLUME_MAX  // 通常为100
...
extern void save_volume_to_flash(int volume);
// 声明应用层提供的队列安全发送函数
extern void uart_send_safe(const char* buf, int len);
typedef enum {
  STATE_IDLE,
  STATE_BROADCAST,
  STATE_PLAY
...
// 定义本地静态变量
static volatile int16_t g_last_doa_angle = -1;

// 提供获取函数
int16_t setting_session_get_last_doa_angle(void)
{
    int16_t angle;
    GIE_DISABLE();
    angle = g_last_doa_angle;
    GIE_ENABLE();
    return angle;
}

void setting_session_clear_doa_angle(void)
{
    GIE_DISABLE();
    g_last_doa_angle = -1;
    GIE_ENABLE();
    LOGT(SETTING_SESSION_TAG, "DOA angle cache cleared");
}
...
static Result _set_volume_min(uni_lasr_result_t *content, uni_bool silent) {
  ...
  save_volume_to_flash(SETTING_VOLUME_MIN);
  if (content->pcm) {
...
static Result _set_volume_max(uni_lasr_result_t *content, uni_bool silent) {
  ...
  save_volume_to_flash(SETTING_VOLUME_MAX);
  if (content->pcm) {
    uni_snprintf(reply, sizeof(reply), "%s", uni_get_random_pcm(content->pcm));
    rc = _reply_volume_set(reply, silent);
  }
...
static Result _set_volume_mid(uni_lasr_result_t *content, uni_bool silent) {
  ...
  save_volume_to_flash(SETTING_VOLUME_MID);
  if (content->pcm) {
    uni_snprintf(reply, sizeof(reply), "%s", uni_get_random_pcm(content->pcm));
    rc = _reply_volume_set(reply, silent);
  }
...
static Result _set_volume_decrease(uni_lasr_result_t *content, uni_bool silent) {
  ...
  if (SETTING_VOLUME_MIN >= volume) {
   ...
   int vol = MediaPlayerVolumeGet();
    bool flag = MediaPlayerIsMute();
    uint8_t cmd_code = 0x55;
    uint8_t tx_buf[9] = {
        0xAA, 0x55, cmd_code, 0x00, vol,
        flag, range,
        0x55, 0xAA
    };
  //  user_uart_send((char*)tx_buf, 9);
  uart_send_safe((char*)tx_buf, 9);
  rc = _reply_volume_set(reply, silent);
  return rc;
}
...
/*  
    LOGT(SETTING_SESSION_TAG, "=== DOA Polling Test Start ===");
    
    for (i = 1; i <= 5; i++) {
        LOGT(SETTING_SESSION_TAG, "Poll #%d: trigger DOA...", i);
        
        // 触发DOA芯片，等待角度数据
        if (doa_wait_for_angle(150) == 0) {
            angle = doa_get_latest_angle();
            LOGT(SETTING_SESSION_TAG, "Poll #%d: angle = %d", i, angle);
            
            // 存储有效角度
         
                GIE_DISABLE();
                g_last_doa_angle = angle;
                GIE_ENABLE();
            
        } else {
            LOGT(SETTING_SESSION_TAG, "Poll #%d: timeout, no angle", i);
            angle = -1;
        }
 ...
 */   
     // 触发DOA芯片，等待角度数据（超时500ms）
    if (doa_wait_for_angle(300) == 0) {
        angle = doa_get_latest_angle();
        LOGT(SETTING_SESSION_TAG, "DOA angle acquired: %d", angle);
        ...
        LOGE(SETTING_SESSION_TAG, "Failed to get DOA angle");
        angle = -1;
    }
