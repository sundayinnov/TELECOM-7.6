...
for i in release debug ; do
    ...
    cd tools/scripts && python2 res_build_tool.py manual
    cd -
    if [ "update"X == "$1"X ]; then
        cd build && make -B OTA_UPDATE=TRUE && make OTA_UPDATE=TRUE
    else
        cd build && make -B && make
    fi
    cd -
    if [ "release"x == "${i}"x ]; then
...
if [ "update"X == "$1"X ]; then
    cd build && make -B OTA_UPDATE=TRUE
else
    cd build && make -B
fi
cd -
