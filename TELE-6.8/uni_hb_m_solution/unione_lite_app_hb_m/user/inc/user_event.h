...
typedef enum {
  ...
  USER_STUDY_START_EVENT,      ///< 学习开始
  USER_STUDY_SUCCESS_EVENT,    ///< 学习成功完成
  USER_STUDY_FAIL_EVENT,       ///< 学习失败（超时或错误次数达阈值）
  USER_STUDY_RESET_EVENT,      ///< 重置学习（清除已学习数据）
  USER_EVENT_MAX
}USER_EVENT_TYPE;
...
/** @brief 对应USER_STUDY_START_EVENT事件*/
typedef struct {
  uni_bool        is_wakeup_study;  ///< TRUE: 唤醒词学习, FALSE: 命令词学习
  const char      *cmd;             ///< 学习的命令词（可选，唤醒词可为"wakeup"）
} event_study_start_t;

/** @brief 对应USER_STUDY_SUCCESS_EVENT / USER_STUDY_FAIL_EVENT事件*/
typedef struct {
  uni_bool        success;          ///< TRUE: 成功, FALSE: 失败
  uni_u8          failed_count;     ///< 失败次数（仅失败时有效）
  const char      *reason;          ///< 失败原因（如"timeout", "max_failures", "similarity"）
} event_study_end_t;

/** @brief 对应USER_STUDY_RESET_EVENT事件*/
typedef struct {
  uni_bool        is_wakeup;        ///< 重置的是唤醒词(TRUE)还是命令词(FALSE)
} event_study_reset_t;
...
typedef union {
  ...
  event_study_start_t     study_start;    ///< 对应USER_STUDY_START_EVENT
  event_study_end_t       study_end;      ///< 对应USER_STUDY_SUCCESS_EVENT / USER_STUDY_FAIL_EVENT
  event_study_reset_t     study_reset;    ///< 对应USER_STUDY_RESET_EVENT
